...
def get_thread_messages(thread_id: str) -> list:
    """Get conversation messages for a specific thread from checkpointer"""
    try:
        logger.info(f"[GET_MESSAGES] Retrieving messages for thread: {thread_id}")
        
        # Method 1: Try using get_state (v0.2+)
        try:
            config = {"configurable": {"thread_id": str(thread_id)}}
            state = chatbot.get_state(config)
            
            logger.info(f"[GET_MESSAGES] State retrieved: {state}")
            
            if state and hasattr(state, 'values') and state.values:
                messages = state.values.get('messages', [])
                logger.info(f"[GET_MESSAGES] Found {len(messages)} messages via get_state()")
                return messages
        except Exception as e1:
            logger.warning(f"[GET_MESSAGES] get_state() failed: {e1}")
        
        # Method 2: Try using checkpointer.get() directly
        try:
            config = {"configurable": {"thread_id": str(thread_id)}}
            checkpoint = checkpointer.get(config)
            
            if checkpoint:
                logger.info(f"[GET_MESSAGES] Checkpoint retrieved directly")
                ...
                logger.info(f"[GET_MESSAGES] Found {len(messages)} messages via checkpointer.get()")
                return messages
        except Exception as e2:
            logger.warning(f"[GET_MESSAGES] checkpointer.get() failed: {e2}")
        
        # Method 3: Try listing all checkpoints and finding the latest for this thread
        try:
            all_checkpoints = checkpointer.list(config={"configurable": {"thread_id": str(thread_id)}})
            checkpoint_list = list(all_checkpoints)
            
            if checkpoint_list:
                # Get the last checkpoint
                latest = checkpoint_list[-1] if checkpoint_list else None
                
                if latest:
                    logger.info(f"[GET_MESSAGES] Latest checkpoint found")
                    
                    # Try to get values from latest checkpoint
                    if hasattr(latest, 'values'):
                        messages = latest.values.get('messages', [])
                    elif hasattr(latest, 'checkpoint') and hasattr(latest.checkpoint, 'values'):
                        messages = latest.checkpoint.values.get('messages', [])
                    else:
                        messages = []
                    
                    logger.info(f"[GET_MESSAGES] Found {len(messages)} messages from latest checkpoint")
                    return messages
        except Exception as e3:
            logger.warning(f"[GET_MESSAGES] checkpointer.list() method failed: {e3}")
        
        logger.warning(f"[GET_MESSAGES] Could not retrieve messages for thread {thread_id}")
        return []
        
    except Exception as e:
        logger.error(f"[GET_MESSAGES] Unexpected error getting thread messages for {thread_id}: {e}")
        import traceback
        traceback.print_exc()
        return []
...
# ============================================
METADATA_FILE = DATA_DIR / "thread_metadata.json"
